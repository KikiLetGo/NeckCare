...
public class FaceMeshResultGlRenderer implements ResultGlRenderer<FaceMeshResult> {
  ...
  private static final float[] IRIS_COLOR = new float[] {1f, 1f, 0.2f, 1f};
  ...
  public void renderResult(FaceMeshResult result, float[] projectionMatrix) {
    ...
    for (int i = 0; i < numFaces; ++i) {
      ...
      FacePose facePose = Utils.findFacePose(result,0);
      drawTest(new float[]{facePose.getNoseVec().getX(),facePose.getNoseVec().getY(),
              facePose.getForwardVec().getX(),facePose.getForwardVec().getY()},
              new float[]{0f,0f,1f,1f},LEFT_EYE_THICKNESS);

      drawTest(new float[]{facePose.getNoseVec().getX(),facePose.getNoseVec().getY(),
                      facePose.getHeadVec().getX(),facePose.getHeadVec().getY()},
              new float[]{1f,0f,0f,1f},LEFT_EYE_THICKNESS);

      // pickOvalPoints(result,0);
    }
  }
